...
foreach ($groupedStudents as $matricule => $group) {
    ...
    foreach ($records as $r) {
        ...
        $historySummary[] = [
            ...
            'classe'              => $r['niveau'] ?? $niveau,
            'semestres_dossier'   => $r['semestres_dossier'] ?? ['S1', 'S2'],
            'moyenne_s1'          => (float)($r['moyenne_s1'] ?? 0),
            'moyenne_s2'          => (float)($r['moyenne_s2'] ?? 0),
            'moyenne_generale'    => (float)($r['moyenne_generale'] ?? 0),
            'credits_s1'          => (int)($r['credits_s1'] ?? 0),
            'credits_s2'          => (int)($r['credits_s2'] ?? 0),
            'credits_total'       => (int)($r['credits_total'] ?? 0),
            'appreciation_s1'     => $r['appreciation_s1'] ?? '',
            'appreciation_s2'     => $r['appreciation_s2'] ?? '',
